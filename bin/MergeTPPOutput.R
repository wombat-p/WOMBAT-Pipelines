...
# reorder columns
ind_cols <- colnames(all_quant)[-which(colnames(all_quant) %in% keep_columns_once)]
all_quant <- all_quant[,c(keep_columns_once, ind_cols[order(ind_cols)])]
ind_cols <- colnames(all_pep_quant)[-which(colnames(all_pep_quant) %in% keep_pep_columns_once)]
all_pep_quant <- all_pep_quant[,c(keep_pep_columns_once, ind_cols[order(ind_cols)])]
...
stand_prot_quant <- all_quant[,grep(paste("^",prot_convert_names, collapse="|", sep=""), colnames(all_quant))]
...
stand_pep_quant <- all_pep_quant[,grep(paste("^",pep_convert_names, collapse="|", sep=""), colnames(all_pep_quant))]
