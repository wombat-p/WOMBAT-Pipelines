...
average_technical_replicates <- function(data, exp_design) {
  ...
  data_out <- data.frame(data[, !grepl("^abundance_", colnames(data))], row.names = rownames(data))
  for (n in all_names) {
...
# Arrange data for PolySTest and make balanced design
arrange_data_for_polystest <- function(proteins) {
  dat_cols <- grep("^abundance_", colnames(proteins), value = T)
  groups <- factor(sub("^abundance_", "", sub("_[0-9]*$", "", dat_cols)))
  new_inds <- order(groups)
  tdat <- proteins[, dat_cols][new_inds]
  groups <- groups[new_inds]
  # Fill each group with empty columns to gain a balanced design
  max_group_size <- max(table(groups))
  ttdat <- matrix(NA, nrow = nrow(tdat), ncol = 0)
  for (i in unique(groups)) {
    ttdat <- cbind(ttdat, tdat[, groups == i, drop = FALSE])
    if (table(groups)[i] < max_group_size) {
      for (j in (table(groups)[i] + 1):max_group_size) {
        ttdat <- cbind(ttdat, NA)
        colnames(ttdat)[ncol(ttdat)] <- paste0("abundance_", i, "_", j)
      }
    }
  }
  # final joining
  proteins <- cbind(
    protein_group = rownames(proteins), proteins[, !grepl("^abundance_", colnames(proteins))],
    ttdat
  )
}
...
write_polystest_yaml <- function(proteins, exp_design, isProt, normalization_method = "none", threads = 1) {
  dat_cols <- grep("^abundance_", colnames(proteins), value = T)
  groups <- factor(sub("^abundance_", "", sub("_[0-9]*$", "", dat_cols)))
  # check whether all groups have the same number of replicates
  if (length(unique(table(groups))) > 1) {
    stop("Not all groups have the same number of replicates")
  }
  pepprot <- ifelse(isProt, "prot", "pep")

  # Define the data to be written into the YAML file
  yaml_data <- list(
    numreps = table(groups)[1],
    numcond = length(unique(groups)),
    normalization = normalization_method,
    paired = FALSE,
    refcond = 0,
    # first column with abundance values
    firstquantcol = min(grep("^abundance_", colnames(proteins))),
    rep_grouped = FALSE,
    csvfile = paste0("stand_", pepprot, "_polystest_in.csv"),
    delim = ",",
    decimal = ".",
    header = TRUE,
    outfile = paste0("stand_", pepprot, "_polystest_out.csv"),
    threads = threads
  )

  # Write the data into the YAML file
  write_yaml(yaml_data, paste0("polystest_", pepprot, ".yml"))
}
...
run_rots <- function(data, groups, comparisons) {
  ...
  for (i in 1:ncol(comparisons)) {
    ...
    RES <- ROTS(data_tmp,
      groups = as.numeric(groups_tmp), log = TRUE,
...
final_assembly <- function(data, contrasts, is_peptides) {
  ...
  for (i in colnames(contrasts)) {
    ttt <- add_cols[, i]
    cname <- sub(" - ", "_vs_", i)
    colnames(ttt) <- paste0(c(
      "log_ratios_", "standard_error_", "degrees_freedom_",
      "t_", "differential_abundance_pvalue_", "differential_abundance_qvalue_"
    ), cname)
