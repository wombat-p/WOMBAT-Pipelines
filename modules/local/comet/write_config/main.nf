process WRITE_CONFIG {
    ...
    if (fixed_mods[0] != null) {
        fixed_mods.each { mod ->
            def tres = "add_" + fixed_map[mod.residue]
            fixed_out += tres + " = " + mod.mass + "\\n"
        }
    }
    ...
    if (var_mods[0]  != null) {
        ...
        var_mods.each { vmod ->
            ...
            def tentry = "variable_mod" + 0 + (i+1) + "=" + vmod.mass + " " + tres + " 0 5 " + (tres.isLowerCase() ? " 0 " : " -1 ") + var_map[vmod.residue] +
                                    " 0 " + (vmod.residue.contains("Phosphorylation") ? 97-976896 : 0) + "\\n"
            var_out += tentry
        }
    }
    ...
    ionmap.each { ion ->
        ions_out += "use_" + ion + "_ions = " + (ion in all_ions ? "1" : "0") + "\\n"
    }
    ...
    if (isotope_range > 3) {
        isotope_range = 3
        println "reduced isotope error range to 3"
    }
